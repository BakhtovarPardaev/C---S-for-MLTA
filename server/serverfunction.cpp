...
QByteArray auth(QJsonObject request) {
    ...
    qDebug() <<dbPassword<<" :    "<<input_password;
...
QByteArray checkTask(QJsonObject request) {
    ...
    if (correctAnswer == "need_check") {
        ...
        switch (taskType) {
        case 1:
            isCorrectAfterCheck = (answer == "да")? true : false;
